...
from backend.services import decode_token, end_session
from backend.models import Session as TrainingSession, Persona, User
...
async def websocket_endpoint(
    ...
    try:
        # ── Session auth ──────────────────────────────────────────────────
        # Resolve the connecting user and verify the session belongs to them.
        # A live training WS is owner-only: the salesperson runs their OWN
        # session. Cross-user / cross-tenant upgrades are rejected here so the
        # pipeline never starts for a session the caller does not own.
        connecting_user = (
            db.query(User).filter(User.id == token_data.user_id).first()
        )
        if connecting_user is None or not connecting_user.is_active:
            await websocket.close(code=4001, reason="Invalid token")
            return

        training_session = (
            db.query(TrainingSession)
            .filter(TrainingSession.id == UUID(session_id))
            .first()
        )
        # Reject if the session is missing or not owned by the connecting user.
        # (Owner-only even for managers/superadmin: they observe via the
        # dashboard REST endpoints, not the live audio socket.)
        if training_session is None or str(training_session.user_id) != token_data.user_id:
