...
def get_session_detail(
    ...
    """
    # Tenant-scoped: salesperson -> own only; manager -> own company; superadmin -> all.
    session = get_session_or_403(db, session_id, current_user)
...
def get_messages(
    ...
    """
    # Tenant-scoped: salesperson -> own only; manager -> own company; superadmin -> all.
    get_session_or_403(db, session_id, current_user)
...
def get_message_audio(
    ...
    # ── Verify session scope (own session, or manager within the company) ──
    get_session_or_403(db, session_id, user)
