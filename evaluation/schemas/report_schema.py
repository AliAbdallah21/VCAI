...
class FactCheckError(BaseModel):
    """One claim the salesperson got wrong."""
    turn_number: Optional[int] = Field(None, description="Turn where the wrong claim was made")
    claim_type: str = Field(..., description="price | size | delivery | down_payment | installment_years | feature | location")
    claimed_value: str = Field(..., description="What the salesperson said")
    correct_value: str = Field(..., description="What the KB says is correct")
    severity: Literal["critical", "minor"] = Field(default="minor")
    explanation_ar: Optional[str] = Field(None, description="Egyptian-Arabic explanation for the trainee")
    property_name: Optional[str] = Field(None, description="Property the claim was about")


class FactCheckSummary(BaseModel):
    """
    Deterministic fact-check results — surfaced as its own section so the
    user immediately sees what the salesperson got wrong vs. the KB.
    Copied directly from rag.fact_checker output, NOT generated by the LLM.
    """
    claims_checked: int = Field(default=0, description="Total claims extracted from transcript")
    accurate_count: int = Field(default=0, description="Claims matching KB")
    inaccurate_count: int = Field(default=0, description="Claims contradicting KB")
    unverifiable_count: int = Field(default=0, description="Claims with no matching KB record")
    accuracy_rate: float = Field(default=0.0, ge=0.0, le=1.0, description="accurate / (accurate + inaccurate)")
    errors: list[FactCheckError] = Field(default_factory=list, description="Specific wrong claims with corrections")
    properties_discussed: list[str] = Field(default_factory=list, description="KB property names mentioned in the call")
...
class FinalReport(BaseModel):
    ...
    # ─── Fact-check (deterministic, not LLM-generated) ────────────────────────

    fact_check: Optional[FactCheckSummary] = Field(
        None,
        description="Salesperson claim accuracy against KB. None if no fact-check ran."
    )
