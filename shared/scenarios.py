...
GenderFit = Literal["male", "female", "any"]
...
SCENARIO_PRESETS: dict[str, dict[str, Any]] = {
    # ── Gender-neutral presets (any persona) ──────────────────────────────────
    ...
    # ── Female-specific presets ───────────────────────────────────────────────
    "single_mother_tight": {
        "buyer_context": "first_time",
        "budget_min": 1_100_000, "budget_max": 1_700_000,
        "timeline": "urgent",
        "gender_fit": "female",
        "must_haves": ["غرفتين على الأقل", "أمان وكمبوند محترم", "قريبة من مدرسة", "مقدم معقول مش كبير"],
    },
    "divorced_fresh_start": {
        "buyer_context": "first_time",
        "budget_min": 900_000, "budget_max": 1_500_000,
        "timeline": "flexible",
        "gender_fit": "female",
        "must_haves": ["شقة باسمها هي", "تقسيط مريح على سنين", "منطقة فيها خدمات"],
    },
    "newlywed_wife_nesting": {
        "buyer_context": "first_time",
        "budget_min": 1_400_000, "budget_max": 2_200_000,
        "timeline": "flexible",
        "gender_fit": "any",
        "must_haves": ["تشطيب كامل أو نصف تشطيب", "تصميم عصري", "قريبة من الأهل", "غرفتين على الأقل"],
    },
    "widowed_downsizer_f": {
        "buyer_context": "downsizer",
        "budget_min": 1_500_000, "budget_max": 2_400_000,
        "timeline": "flexible",
        "gender_fit": "female",
        "must_haves": ["دور أرضي أو واطي مع أسانسير", "منطقة هادية وآمنة", "مصاريف صيانة قليلة", "قريبة من ولادها"],
    },
    "female_investor": {
        "buyer_context": "investor",
        ...
        "timeline": "flexible",
        "gender_fit": "female",
        "must_haves": ["موقع بيرتفع سعره مع الوقت", "إمكانية تأجير بعائد كويس", "كمبوند له سمعة"],
    },
    "family_buyer_with_kids": {
        "buyer_context": "family_upgrade",
        "budget_min": 2_500_000, "budget_max": 3_800_000,
        "timeline": "flexible",
        "gender_fit": "any",
        "must_haves": ["٣ غرف نوم على الأقل", "كمبوند فيه أمان ومساحات خضرا", "قريبة من مدارس كويسة", "جراج للعربية"],
    },
}
...
def _finalize(scenario: dict[str, Any]) -> dict[str, Any]:
    ...
    scenario.setdefault("gender_fit", "any")
    return scenario
...
def list_presets_for_gender(gender: str) -> list[dict[str, Any]]:
    """Return presets compatible with the given persona gender (male/female).

    A preset is included when its gender_fit is 'any' or matches exactly.
    """
    out = []
    for pid, preset in SCENARIO_PRESETS.items():
        fit = preset.get("gender_fit", "any")
        if fit == "any" or fit == gender:
            sc = scenario_from_preset(pid)
            sc["preset_id"] = pid
            out.append(sc)
    return out
